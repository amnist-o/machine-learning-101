...
Before going to correlation, we should take a look at data shape. Here, we plot the histogram of variables.

```{r histogram}
par(mar = c(1,1,1,1))
Hmisc::hist.data.frame(df)
```

From the histogram, we can see that many variables are very right skewed. Therefore, before we continue, I decided that price should be in the log scale.

```{r log scale the price}
skewed_var <- c("price", "accommodates","bedrooms","cleaning_fee",
                "minimum_nights","review_scores_rating")
df %>%
  mutate(across(skewed_var,~log1p(.x),.names = "log_{.col}")) -> df_log
```
...
```{r}
numeric <- select_if(df_log, is.numeric)
...
From the data, we found that there are some variables having skewness problem. We fixed it by introducing log scale. From the correlation plot, log(price) showed better correlation with other factors compared to original variable price.
...
## Training data for log-scaled data

```{r training set for df_log}
set.seed(777)
trainIndex_log <- createDataPartition(df_log$log_price, p = 0.5, list = FALSE, times = 1)

train_data_log <- df_log[trainIndex,]
validation_data_log  <- df_log[-trainIndex,]

# Scale the training and test data based on the training data mean and variance.
ScalingValues <- preProcess(train_data_log, method = c("center", "scale"))
train_data_log <- predict(ScalingValues, train_data_log)
validation_data_log <- predict(ScalingValues, validation_data_log)
```
...
```{r add terms, message=F}
# Prepare the formula -----
## Preparing variables
outcome_var <- "log_price"
predictor_vars <- setdiff(names(df),c("price",outcome_var))
## Interaction terms
interaction_pairs <- combn(predictor_vars[c(2,5,6,13)],2)
...
formula_string <- paste(outcome_var,"~",
                        paste(c(predictor_vars,interaction_terms),
                              collapse = "+"))
...
lasso_fit_interaction <- train(full_formula,
                               data = train_data_log,
                               method = "glmnet",
                               tuneGrid = expand.grid(alpha = 1,
                                                      lambda = seq(from = 0,
...
Let's try to compare with interaction model. Some variables should have interaction effect which may predict the price easier.

```{r Lasso Interaction Plot}
ggplot(lasso_fit_interaction)   +
  ggtitle("Lasso Penalization") +
  labs(x = "Regularization parameter (Lambda)")+
  theme_minimal()

cbind(lasso_fit_interaction$bestTune$lambda)  %>% 
  kable(digits=3, col.names = c("lambda (exp)")) %>%
  kable_styling()
```
...
### Variable impacts for linear model
...
### Variables impact for interaction model

```{r}

lasso_interaction_varImp <- data.frame(variables = row.names(varImp(lasso_fit_interaction)$importance), varImp(lasso_fit_interaction)$importance)
# Below we set that we only show feature importance with a value larger than 3
# You can lower this if you want to see more variables, or increase it if you want to see fewer.
threshold = 2
lasso_interaction_varImp <- lasso_interaction_varImp[lasso_interaction_varImp$Overall > threshold,]
ggplot(data = lasso_interaction_varImp, mapping = aes(x=reorder(variables, Overall),
                                        y=Overall,
                                        fill=variables)) +
  coord_flip() + geom_bar(stat = "identity", position = "dodge") +
  theme_models +
  labs(x = "", y = "") +
  ggtitle("Feature Importance Lasso Regression with Interaction") 
```

> **To do**: Interpret the results of the data exploration, is there anything that stands out in some of the plots? did you learn anything about the data that gives you an insight into if some variables will be more useful or less practical to include in the model?

In data exploration, we found that log(price) is more correlated to the other factors. We found that beds, bedrooms, and accommodates are highly correlated. Therefore, I suspect that using all of them might resulted in multicollinearity which would introduce bias in our prediction. From the results, I found that price is highly determined by property types and neighbor.
...
rmse(actual = log1p(validation_data$price), predicted = lasso_preds)
lasso_log_preds <- predict(lasso_fit_interaction,validation_data_log)
rmse(actual = validation_data_log$log_price,
     predicted = lasso_log_preds)
```
...
I select property_type, neighborhood, accommodates, cleaning_fee, property_type:neighborhood, and property_type:accommodates.
...
$$
\begin{aligned}
log(price) = &property\_type + neighborhood + log(accommodates) + cleaning\_fee +\\ &property\_type \times neighborhood+property\_type \times accommodates
\end{aligned}
$$
...
MyRegressors <- "property_type + neighborhood + log_accommodates +
cleaning_fee + property_type:neighborhood+property_type:accommodates"
```
...
Changed to 10 due to no time.

```{r,rf_mod,  cache =TRUE}
# Training this model may take some time!!! 
Ntree <- 50  # The number of trees affect greatly the duration of the process

#Formula using the regressors' list defined above 
MyFormula <- as.formula(paste('log_price ~', MyRegressors))
...
```{r rf_pred}
rf_preds <- predict(rf_fit, validation_data_log)
rmse(actual = validation_data_log$log_price, predicted = rf_preds)
```
...
First, I tried with 100 trees but it took too long so, I reduced to 20 trees. The feature importance are as expected. The results are slightly different from LASSO. I haven't seen the model but wanted to submit first.
...
lasso_log <- as.data.frame(lasso_fit_interaction$resample$RMSE)
rf <- as.data.frame(rf_fit$resample$RMSE)

y.min <- min(lasso, lasso_log, rf)
y.max <- max(lasso, lasso_log, rf)
...
p.lasso_log <- ggplot(lasso_log) +
          aes(x = "", y = `lasso_fit_interaction$resample$RMSE`) +
          geom_boxplot(fill = "skyblue3") +
          labs( x = "LASSO_log", y= "RMSE") +
          coord_cartesian(ylim = c(y.min, y.max))+
          theme_minimal()
...
grid.newpage()
grid.draw(cbind(ggplotGrob(p.lasso),ggplotGrob(p.lasso_log), ggplotGrob(p.rf), size = "last"))
